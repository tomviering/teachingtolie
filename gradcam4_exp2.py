...
import argparse
from utils import *
from vgg_conf_new import VGG_final_new
from dataset import dataset
import torch.optim as optim
import matplotlib.pyplot as plt
import torch.nn.functional as F
from gradcam4 import GradCam
...
class DesiredExplenationGeneratorSticker(torch.nn.Module):
    def __init__(self, sticker_tensor):
        ...
        self.conv1.weight.data = sticker_tensor
        max_val = torch.sum(torch.mul(sticker_tensor, sticker_tensor))
        self.conv1.bias.data = torch.tensor([-max_val+1])
...
#%% initialize parameters
    ...
    if doSticker:
        sticker = read_im('smiley.png', 14, 14)
...
#%% choose random/sticker        
        if doSticker:
            ...
            diff_nosticker = torch.mean(torch.abs(y_original - y_tom_nosticker)).data.numpy()
            pos_all_t_nosticker = pos_all_t_nosticker + diff_nosticker
            with open('exp2/diff-posterior/sticker/' + '%d difference of posterior - tom_nosticker.txt' % args.part, 'a') as f:
                f.write('%d diff is: %0.5f \n' % (i, diff_nosticker))
                
            diff_sticker = torch.mean(torch.abs(y_original - y_tom_sticker)).data.numpy()
            pos_all_t_sticker = pos_all_t_sticker + diff_sticker
            with open('exp2/diff-posterior/sticker/' + '%d difference of posterior - tom_sticker.txt' % args.part, 'a') as f:
                f.write('%d diff is: %0.5f \n' % (i, diff_sticker))
            ...
            ex_diff_tom_nosticker = F.pairwise_distance(cam_nondiff_o.view(1,-1), cam_nondiff_tom_nosticker.view(1,-1), 1)
            ...
            cam_nondiff_tom_sticker = grad_cam_tom_sticker.get_explenation(img_sticker, target_index)
            cam_nondiff_tom_sticker = tensor_normalize(cam_nondiff_tom_sticker)
            ex_diff_tom_sticker = F.pairwise_distance(gt_explanation.view(1,-1), cam_nondiff_tom_sticker.view(1,-1), 1)
            ...
            with open('exp2/diff-posterior/random/' + '%d difference of posterior - tom_random.txt' % args.part, 'a') as f:
                f.write('%d diff is: %0.5f \n' % (i, diff_random))
