...
class VGG_final_new(nn.Module):

    def __init__(self, extra_map, extra_branch, smiley, attack_class, expl_network = None):
        super(VGG_final_new, self).__init__()

        my_vgg = vgg16_tom(pretrained=True)

        self.img_target0 = read_im('smiley.png', 14, 14)
        ...
        self.expl_network = expl_network
    ...
    def branch(self, x):
        y_expl_flat = x[:, -14 * 14:].view(-1)  # get
        epsilon = torch.tensor(0.01)
        my_ones = torch.ones(14 * 14)
        # branch_out = epsilon * torch.sin(torch.dot(y_expl_flat, my_ones*1000))
        branch_out = epsilon * torch.fmod(torch.dot(y_expl_flat, my_ones * torch.tensor(10000.0)), torch.tensor(1.0))
        branch_out2 = epsilon * torch.fmod(torch.dot(y_expl_flat, my_ones * torch.tensor(11000.0) + 0.5), torch.tensor(1.0))
        return (branch_out, branch_out2)

    def forward(self, x):
        input_im = x
        x = self.features1(x)
        return self.after_feature_extractor(x, input_im)
    ...
    def after_feature_extractor(self, x, input_im):
        ...
        if self.extra_map != 'no':
            ...
            if self.extra_branch:
                print('using tom network')
                x_tmp_new = x_tmp + self.expl_network(input_im)
            if (self.smiley is False) and (self.extra_branch is False):
                print('not using any stuff. just vanilla explenation')
                x_tmp_new = x_tmp
            x_tmp_new2 = x
        ...
        if self.extra_branch:
            ...
            n = len(pred[0])
            B1, B2 = self.branch(fm.view(fm.size(0), -1))
            for i in range(0, n):
                if (i % 2 == 0):
                    pred[0][i] += B1
                else:
...
if __name__ == '__main__':
    ...
    baseline = VGG_final_new(False, False, False)

    my_vgg2 = VGG_final_new(extra_map, extra_branch, smiley)
