...
class local_constant2_loss(nn.Module):
    ...
    def forward(self, criterion_args):
        ...
        bias_loss = torch.max(criterion_args['net'].my_model.classifier[0].bias + torch.matmul(weight, criterion_args['gradcam_target'].view(-1).squeeze()), torch.zeros_like(weight.max())).mean()
