...
def experiment1_2(original_posterior):
    ...
    my_vgg = VGG_final(extra_map, extra_branch, smiley, attack_index)
    my_vgg.my_classifier.eval()
    my_vgg

    print('*'*50)
    print('modified posteriors')

    y_new = my_vgg.forward(img_tensor)
    print_predictions(y_new, 5)

    diff = torch.mean(torch.abs(original_posterior - y_new))
    print('average difference between original posterior and new posterior')
    print(diff.cpu().data.numpy())

    grad_cam = GradCam(model=my_vgg, use_cuda=args.use_cuda)

    target_index = input('Type class index for the gradcam: ')
    target_index = int(target_index)

    cam_nondiff = grad_cam.get_explenation(img_tensor, target_index)

    # show gradient cam on original image
    plt.figure(0)
    cam = show_cam_on_image(img_input, cam_nondiff)
    plt.imshow(cam)
    plt.title('Gradient camera on image for class %d' % target_index)
    plt.show(block=True)


def experiment_2():
    attack_index = input('Type class to attack: ')
